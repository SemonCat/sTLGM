...
import com.thu.digi256.Input.GestureListener;
...
public class MainView extends View {
    ...
    public void drawCells(Canvas canvas) {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                if (currentTile != null) {
                    int imageType = currentTile.isQuestion() ? 0 : 1;
                    //Get and represent the value of the tile
                    int value = currentTile.getValue();
                    int index = log2(value);
                    //Log.d("","value:"+value+",index:"+index);
                    //index = index>=bitmapCell.length ? bitmapCell.length-1 : index;
                    ...
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        ...
                        if (aCell.getAnimationType() == MainGame.SPAWN_ANIMATION) { // Spawning animation
                            ...
                            bitmapCell[index][imageType].setBounds((int) (sX + cellScaleSize), (int) (sY + cellScaleSize), (int) (eX - cellScaleSize), (int) (eY - cellScaleSize));
                            bitmapCell[index][imageType].draw(canvas);
                            ...
                            bitmapCell[tempIndex][imageType].setBounds(sX + dX, sY + dY, eX + dX, eY + dY);
                            if (tempIndex < bitmapCell.length) {
                                bitmapCell[tempIndex][imageType].draw(canvas);
                            } else {
                                bitmapCell[bitmapCell.length - 1][imageType].draw(canvas);
                            }
                        }
                        animated = true;
                    }
                    ...
                    if (!animated) {
                        bitmapCell[index][imageType].setBounds(sX, sY, eX, eY);
    ...
    public void createBitmapCells() {
        ...
        for (int xx = 0; xx < bitmapCell.length; xx++) {
            ...
            */
            ...
            if (xx==0){
                bitmapCell[xx][0] = (BitmapDrawable)getResources().getDrawable(R.drawable.opt_m51_f);
                bitmapCell[xx][1] = (BitmapDrawable)getResources().getDrawable(R.drawable.opt_m51_f);
            }else{
                Bitmap bitmap1 = Bitmap.createBitmap(cellSize, cellSize, Bitmap.Config.ARGB_8888);
                Canvas canvas1 = new Canvas(bitmap1);
                drawDrawable(canvas1, cellRectangle[xx-1], 0, 0, cellSize, cellSize);


                drawDrawable(canvas1, cellContent[xx-1][0], 0, 0, cellSize, cellSize);

                Bitmap bitmap2 = Bitmap.createBitmap(cellSize, cellSize, Bitmap.Config.ARGB_8888);
                Canvas canvas2 = new Canvas(bitmap2);
                drawDrawable(canvas2, cellRectangle[xx-1], 0, 0, cellSize, cellSize);


                drawDrawable(canvas2, cellContent[xx-1][1], 0, 0, cellSize, cellSize);

                bitmapCell[xx][0] = new BitmapDrawable(resources, bitmap1);
                bitmapCell[xx][1] = new BitmapDrawable(resources, bitmap2);
            }
        }
    }
    ...
    private void init(Context context){
        ...
        try {
            ...
            /*
            cellContent = new Drawable[][]{
                    {
                            resources.getDrawable(R.drawable.opt_m41_f_03),
                            resources.getDrawable(R.drawable.obj_m_assailant)
                    },

                    {
                            resources.getDrawable(R.drawable.obj_m_collectors),
                            resources.getDrawable(R.drawable.opt_m41_f_01)
                    },
                    {
                            resources.getDrawable(R.drawable.obj_m_trimmer),
                            resources.getDrawable(R.drawable.opt_m41_f_02),
                    }
            };


            bitmapCell = new BitmapDrawable[cellContent.length][2];
            */
            cellContent = new Drawable[][]{};
            bitmapCell = new BitmapDrawable[0][2];
    ...
    public void setGameResource(Drawable[][] GameResource){
        cellContent = GameResource;
        bitmapCell = new BitmapDrawable[cellContent.length+1][2];
        invalidate();
    }
